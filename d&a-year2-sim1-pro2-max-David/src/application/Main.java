	...
	static Integer[] sortString(String[] args) {
		...
		while (swapped == true) {
			...
			for (int i = 0; i < args.length - 1; i++) // LOOPS THROUGH TITLES
			{
				...
				{
					String temp = args[i]; // Swaps similar to regular bubble sort but uses a seperate index to return
					int tempInt = index[i]; // an integer array which corresponds to the display.
					index[i] = (index[(i + 1)]);
					args[i] = (args[(i + 1)]);
					index[(i + 1)] = tempInt;
					args[(i + 1)] = temp;
